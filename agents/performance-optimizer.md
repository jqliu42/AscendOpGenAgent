...
1. **禁止一次性尝试多个优化点**：每轮迭代只能选择一个优化点进行尝试
2. **优化点排序原则**：根据确定性和难易程度排序，从简单到难
   - **高确定性/简单优化**（优先尝试）：
   - **中等确定性优化**：
   - **低确定性/复杂优化**（最后尝试）：
3. **基线更新机制**：
   - 如果某个优化点尝试成功，该版本代码将作为下一轮优化的基线
   - 如果优化点尝试失败或无效果，回退到上一轮的基线代码，继续尝试下一个优化点
4. **优化点记录**：在 `history_attempts` 中记录已尝试的优化点，避免重复尝试
...
**⚠️ 关键要求**：
- **每轮迭代只应用一个优化点**，禁止一次性应用多个优化策略
- 根据 Step 3 中确定的优化点优先级，选择当前最优先且未尝试过的优化点
- 在代码注释中明确标注本轮应用的优化策略名称
...
| **优化策略选择** | **每轮迭代只能尝试一个优化点，禁止一次性尝试多个优化点** |
| **优化点排序** | **必须按确定性从高到低、难度从简单到难的顺序尝试优化点** |
| **基线更新** | **优化成功后，该版本作为下一轮优化的基线** |
